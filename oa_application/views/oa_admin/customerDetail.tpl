    ...
    <tbody>
      ...
      {else}
       <tr>
        ...
        <td>{$customerInfo['customer_pregnant_week']}</td>
      </tr>
       <tr>
       ...
       <tr>
        <th class="text-r" width="120">服务级别：</th>
        <td>{$serviceLevel2[$customerInfo['customer_service_level']]}</td>
      </tr>
      {/if}
      <tr>
        <th class="text-r">{if checkRight('customer_follow_add')}<a id="addFollow" title="增加关注我的用户" href="javascript:;" style="text-decoration:none"><i class="icon-plus"></i></a>{/if}关注我的用户：</th>
        <td>
        	{if empty($followInfo)}
        		暂无关注我的用户
        	{else}
        		<table class="table table-bg table-border table-bordered">
        			...
        			{foreach $followInfo as $item}
        				<tr>
        					<td width="70%">
        						{if $item['user_nickname']}{$item['user_nickname']}_{/if}{$item['user_phone']}({$item['relationship']})
        					</td>
        					<td>
...
<ul>
<%#userList%>
<li uid="<%user_id%>"><%#user_nickname%><%user_nickname%>_<%/user_nickname%><%user_phone%></li>
<%/userList%>
</ul>
</script>
